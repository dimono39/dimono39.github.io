...
const DailyMenuModule = (function() {
    ...
    function getMenuColor(menuNumber) {
        return MENU_COLORS[menuNumber] || MENU_COLORS[1];
    }
    ...
    function getDayFromMenuNumber(menuNumber) {
        ...
        if (menuNumber <= 5) {
            return { week: 1, day: menuNumber };
        } else if (menuNumber <= 10) {
            return { week: 2, day: menuNumber - 5 };
        }
        
        // Для номеров > 10 ищем в данных
        const allDays = [];
    ...
    function updateMenuSelectorUI() {
        ...
        const maxNum = CONFIG.MAX_MENU_NUMBER;
        let html = '';
        ...
        for (let i = 1; i <= maxNum; i++) {
            ...
            html += `
                <div class="menu-number-btn ${isActive ? 'active' : ''} ${!hasData ? 'opacity-50' : ''}" 
                    ...
                    <div style="width: 100%; height: 3px; background: ${color.primary}; border-radius: 2px; margin-top: 4px;"></div>
                </div>
            `;
        }
